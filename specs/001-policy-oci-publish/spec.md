...
- **Promotion to the public ComplyTime registry on Quay**: The **target** path from **staging** (`ghcr.io`) to `quay.io/…` is the organization’s **reusable** workflow in **complytime/org-infra** — for example [`.github/workflows/resuable_publish_quay.yml`](https://github.com/complytime/org-infra/blob/main/.github/workflows/resuable_publish_quay.yml) — with the **inputs** and **secrets** that workflow defines. **As of PR #19**, the **caller** instead delegates to a **composite** that implements the same **ORAS copy + cosign** flow inline (see [contracts/publish-pipeline.md](contracts/publish-pipeline.md)); migrating to a literal **`workflow_call`** to that reusable (or a thin wrapper) is the **convergence** work item, not a duplicate script in *this* repo.
